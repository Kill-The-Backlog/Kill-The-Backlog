...
import { restartSessionPreview } from "#lib/.server/session-preview/lifecycle.js";
...
export const sessionPreviewStarterWorker = defineWorker<JobData>(
  "session-preview-starter",
  async (job) => {
    ...
    try {
      ...
      await restartSessionPreview({ clonePath, job, sandbox, sessionId });
    } catch (error) {
      ...
      await job.log(`[preview] worker failed: ${message}`);
      throw error;
    }
  },
  {
