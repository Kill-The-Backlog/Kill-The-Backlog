...
export type AnyWorker = Worker<unknown, unknown>;
...
>(
  ...
  const queue = new BullQueue<JobData, JobResult>(name, {
  ...
  const worker = new BullWorker<JobData, JobResult>(name, handler, {
    autorun: false,
